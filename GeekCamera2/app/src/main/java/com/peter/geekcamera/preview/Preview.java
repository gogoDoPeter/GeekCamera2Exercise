package com.peter.geekcamera.preview;

import com.peter.geekcamera.cameracontroller.RawImage;
//import com.deepinout.geekcamera.MainActivity;
import com.peter.geekcamera.MyDebug;
import com.peter.geekcamera.R;
import com.deepinout.geekcamera.ScriptC_histogram_compute;
import com.peter.geekcamera.TakePhoto;
import com.peter.geekcamera.ToastBoxer;
import com.peter.geekcamera.cameracontroller.CameraController;
import com.peter.geekcamera.cameracontroller.CameraController1;
import com.peter.geekcamera.cameracontroller.CameraController2;
import com.peter.geekcamera.cameracontroller.CameraControllerException;
import com.peter.geekcamera.cameracontroller.CameraControllerManager;
import com.peter.geekcamera.cameracontroller.CameraControllerManager1;
import com.peter.geekcamera.cameracontroller.CameraControllerManager2;
import com.peter.geekcamera.preview.ApplicationInterface.NoFreeStorageException;
import com.peter.geekcamera.preview.camerasurface.CameraSurface;
import com.peter.geekcamera.preview.camerasurface.MySurfaceView;
import com.peter.geekcamera.preview.camerasurface.MyTextureView;
...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private List<String> supported_capabilitys;
    ...
    private boolean take_photo_after_autofocus; // set to take a photo when the in-progress autofocus has completed; if setting, remember to call mCameraController.setCaptureFollowAutofocusHint()
    ...
    /* If the user touches to focus in continuous mode, and in photo mode, we switch the mCameraController to autofocus mode.
    ...
    public boolean touchEvent(MotionEvent event) {
        ...
        if( mCameraController != null && !this.using_face_detection && !was_paused ) {
            this.has_focus_area = false;
            ArrayList<CameraController.Area> areas = getAreas(event.getX(), event.getY());
            if( mCameraController.setFocusAndMeteringArea(areas) ) {
                if( MyDebug.LOG )
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            if( Preview.this.mCameraController != null && Preview.this.has_zoom ) {
    ...
    public void clearFocusAreas() {
        ...
        mCameraController.clearFocusAndMetering();
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture texture, int width, int height) {
        ...
        if( mCameraController != null ) {
            ...
            if( mSetPreviewSize && (width != preview_w || height != preview_h) ) {
                ...
                mCameraController.updatePreviewTexture();
            }
        }
    ...
    private void reconnectCamera(boolean quiet) {
        ...
        if( mCameraController != null ) { // just to be safe
            try {
                mCameraController.reconnect();
    ...
    private void closeCamera(boolean async, final CloseCameraCallback closeCameraCallback) {
        ...
        synchronized( this ) {
            ...
            // no need to call mCameraController.setCaptureFollowAutofocusHint() as we're closing the camera
        }
        ...
        if( mCameraController != null ) {
            ...
            if( mCameraController != null ) {
                ...
                pausePreview(false);
                // we set mCameraController to null before starting background thread, so that other callers won't try
                // to use it
    ...
    /**
     * @param stop_preview Whether to call mCameraController.stopPreview(). Normally this should be
    ...
    /** Try to open the camera. Should only be called if mCameraController==null.
    ...
    private void openCamera() {
        ...
        synchronized( this ) {
            // synchronise for consistency (keep FindBugs happy)
            take_photo_after_autofocus = false;
            // no need to call mCameraController.setCaptureFollowAutofocusHint() as we're opening the camera
        }
        set_flash_value_after_autofocus = "";
        successfully_focused = false;
        preview_targetRatio = 0.0;
        scene_modes = null;
        supported_capabilitys = null;
        ...
        if( use_background_thread ) {
            ...
            this.mCameraController = openCameraCore(cameraId);
            if( MyDebug.LOG ) {
    ...
    private CameraController openCameraCore(int cameraId) {
        ...
        // We pass a camera controller back to the UI thread rather than assigning to mCameraController here, because:
        // * If we set mCameraController directly, we'd need to synchronize, otherwise risk of memory barrier issues
        // * Risk of race conditions if UI thread accesses mCameraController before we have called cameraOpened().
    ...
    public void setupCamera(boolean take_photo) {
        ...
        if( this.is_video ) {
            ...
            mCameraController.setTonemapProfile(tonemap_profile, video_log_profile_strength, video_gamma);
        }
        ...
        mCameraController.setVideoHighSpeed(is_video && video_high_speed);

        if( do_startup_focus && using_android_l && mCameraController.supportsAutoFocus() ) {
            ...
            set_flash_value_after_autofocus = "";
            String old_flash_value = mCameraController.getFlashValue();
            ...
            if( old_flash_value.length() > 0 && !old_flash_value.equals("flash_off") && !old_flash_value.equals("flash_torch") ) {
                ...
                mCameraController.setFlashValue("flash_off");
            }
            if( MyDebug.LOG )
        ...
        if( this.supports_raw && applicationInterface.getRawPref() != ApplicationInterface.RawPref.RAWPREF_JPEG_ONLY ) {
            mCameraController.setRaw(true, applicationInterface.getMaxRawImages());
        }
        else {
            mCameraController.setRaw(false, 0);
        }

        setupBurstMode();

        if( mCameraController.isBurstOrExpo() ) {
            if( MyDebug.LOG )
        ...
        mCameraController.setOptimiseAEForDRO( applicationInterface.getOptimiseAEForDROPref() );
        ...
        if( this.supports_expo_bracketing && applicationInterface.isExpoBracketingPref() ) {
            ...
            if( applicationInterface.getBurstForNoiseReduction() ) {
                if( this.supports_exposure_time ) { // noise reduction mode also needs manual exposure
                    ...
                    mCameraController.setBurstType(CameraController.BurstType.BURSTTYPE_NORMAL);
                    mCameraController.setBurstForNoiseReduction(true, nr_mode==ApplicationInterface.NRModePref.NRMODE_LOW_LIGHT);
                }
                else {
                ...
                mCameraController.setBurstType(CameraController.BurstType.BURSTTYPE_NORMAL);
                mCameraController.setBurstForNoiseReduction(false, false);
                mCameraController.setBurstNImages(applicationInterface.getBurstNImages());
            }
        }
        else {
            mCameraController.setBurstType(CameraController.BurstType.BURSTTYPE_NONE);
    ...
    private void initCameraParameters() throws CameraControllerException {
            ...
            Log.d(TAG, "initCameraParameters()");
        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setSceneMode(value);
...

        {
            ...
            CameraController.CameraFeatures camera_features = mCameraController.getCameraFeatures();
    ...
    private void setupCameraParameters() {
...

        {
            ...
            if( this.using_face_detection ) {
                    ...
                    public void onFaceDetection(final CameraController.Face[] faces) {
                        ...
                        activity.runOnUiThread(new Runnable() {
                            public void run() {
                                ...
                                // (otherwise can have crashes if mCameraController becomes null in the meantime)
                ...
                mCameraController.setFaceDetectionListener(new MyFaceDetectionListener());
            }
        }
        if( MyDebug.LOG ) {
...

        {
            ...
            if( this.supports_video_stabilization ) {
                ...
                if( MyDebug.LOG )
                    ...
                    mCameraController.setVideoStabilization(using_video_stabilization);
            }
            if( MyDebug.LOG )
...

        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setColorEffect(value);
            if( supported_values != null ) {
...

        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setWhiteBalance(value);
            if( supported_values != null ) {
                ...
                if( supported_values.selected_value.equals("manual") && this.supports_white_balance_temperature ) {
                    ...
                    mCameraController.setWhiteBalanceTemperature(temperature);
                    if( MyDebug.LOG )
                        Log.d(TAG, "saved white balance: " + value);
                }
            }
...

        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setAntiBanding(value);
...

        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setEdgeMode(value);
...

        {
            ...
            CameraController.SupportedValues supported_values = mCameraController.setNoiseReductionMode(value);
        ...
        if( supports_iso_range ) {
            ...
            if( value.equals(CameraController.ISO_DEFAULT) ) {
                if( MyDebug.LOG )
                    ...
                    mCameraController.setManualISO(false, 0);
            }
            else {
                ...
                if( iso >= 0 ) {
                    ...
                    if( MyDebug.LOG )
                        Log.d(TAG, "iso: " + iso);
                        mCameraController.setManualISO(true, iso);
                }
                else {
                    // failed to parse
                    mCameraController.setManualISO(false, 0);
            ...
            CameraController.SupportedValues supported_values = mCameraController.setISO(value);
            if( supported_values != null ) {
        ...
        if( is_manual_iso ) {
            if( supports_exposure_time ) {
                ...
                if( exposure_time_value < getMinimumExposureTime() )
                    ...
                    mCameraController.setExposureTime(exposure_time_value);
                // now save
...

        {
            ...
            if( min_exposure != 0 || max_exposure != 0 ) {
                ...
                if( !is_manual_iso ) {
                    ...
                    mCameraController.setExposureCompensation(exposure);
                    // now save, so it's available for PreferenceActivity
            ...
            if( aperture > 0.0f ) {
                ...
                for(float this_aperture : supported_apertures) {
                    if( this_aperture == aperture ) {
                        mCameraController.setAperture(aperture);
                    }
                }
...

        {
            ...
            if( MyDebug.LOG )
                ...
                mCameraController.setJpegQuality(image_quality);
        }
        if( MyDebug.LOG ) {
...

        {
            ...
            if( focus_distance_value < 0.0f )
                ...
                mCameraController.setFocusDistance(focus_distance_value);
                mCameraController.setFocusBracketingSourceDistance(focus_distance_value);
            // now save
        ...
        }
        {
            ...
            if( focus_distance_value < 0.0f )
                ...
                focus_distance_value = minimum_focus_distance;
                mCameraController.setFocusBracketingTargetDistance(focus_distance_value);
            // now save
    ...
    private void setPreviewSize() {
        ...
        if( new_size != null ) {
            mCameraController.setPictureSize(new_size.width, new_size.height);
        }
    ...
    private double calculateTargetRatioForPreview(Point display_size) {
        ...
        if( preview_size.equals("preference_preview_size_wysiwyg") || this.is_video ) {
            if( this.is_video ) {
                ...
                CameraController.Size picture_size = mCameraController.getPictureSize();
                if( MyDebug.LOG )
    ...
    public void setCameraDisplayOrientation() {
        ...
        if( using_android_l ) {
            ...
            if( MyDebug.LOG )
                Log.d(TAG, "    degrees = " + degrees);
            // note the code to make the rotation relative to the camera sensor is done in mCameraController.setDisplayOrientation()
            mCameraController.setDisplayOrientation(degrees);
        }
    }
    ...
    private void onOrientationChanged(int orientation) {
        ...
        int new_rotation;
        int camera_orientation = mCameraController.getCameraOrientation();
        if( (mCameraController.getFacing() == CameraController.Facing.FACING_FRONT) ) {
    ...
    public int getScaledZoomFactor(float scale_factor) {
        ...
        if( this.mCameraController != null && this.has_zoom ) {
            int zoom_factor = mCameraController.getZoom();
    ...
    public void zoomTo(int new_zoom_factor) {
        ...
        if( mCameraController != null ) {
            if( this.has_zoom ) {
                // don't cancelAutoFocus() here, otherwise we get sluggish zoom behaviour on Camera2 API
                mCameraController.setZoom(new_zoom_factor);
    ...
    public void setFocusDistance(float new_focus_distance, boolean is_target_distance) {
        ...
        if( mCameraController != null ) {
            ...
            if( is_target_distance ) {
                focus_changed = true;
                mCameraController.setFocusBracketingTargetDistance(new_focus_distance);
                // also set the focus distance, so the user can see what the target distance looks like
                mCameraController.setFocusDistance(new_focus_distance);
            }
            else if( mCameraController.setFocusDistance(new_focus_distance) ) {
                focus_changed = true;
                mCameraController.setFocusBracketingSourceDistance(new_focus_distance);
            }
    ...
    public void stoppedSettingFocusDistance(boolean is_target_distance) {
        ...
        if( is_target_distance && mCameraController != null ) {
            if( MyDebug.LOG )
                Log.d(TAG, "set manual focus distance back to start");
                mCameraController.setFocusDistance( mCameraController.getFocusBracketingSourceDistance() );
        }
    }
    ...
    public void setExposure(int new_exposure) {
        ...
        if( mCameraController != null && ( min_exposure != 0 || max_exposure != 0 ) ) {
            cancelAutoFocus();
            if( new_exposure < min_exposure )
                new_exposure = min_exposure;
            else if( new_exposure > max_exposure )
                new_exposure = max_exposure;
            if( mCameraController.setExposureCompensation(new_exposure) ) {
                // now save
    ...
    public void setWhiteBalanceTemperature(int new_temperature) {
        ...
        if( mCameraController != null ) {
            if( mCameraController.setWhiteBalanceTemperature(new_temperature) ) {
                // now save
    ...
    public void setISO(int new_iso) {
        ...
        if( mCameraController != null && supports_iso_range ) {
            if( new_iso < min_iso )
                new_iso = min_iso;
            else if( new_iso > max_iso )
                new_iso = max_iso;
            if( mCameraController.setISO(new_iso) ) {
                // now save
    ...
    public void setExposureTime(long new_exposure_time) {
        ...
        if( mCameraController != null && supports_exposure_time ) {
            ...
            if( mCameraController.setExposureTime(new_exposure_time) ) {
                // now save
    ...
    private void setPreviewFps() {
        ...
        VideoProfile profile = getVideoProfile();
        List<int []> fps_ranges = mCameraController.getSupportedPreviewFpsRange();
        ...
        if( selected_fps != null ) {
            if( MyDebug.LOG )
                ...
                mCameraController.setPreviewFpsRange(selected_fps[0], selected_fps[1]);
        }
        else if( using_android_l ) {
            mCameraController.clearPreviewFpsRange();
        }
    }
    ...
    private boolean focusIsVideo() {
        if( mCameraController != null ) {
            return mCameraController.focusIsVideo();
        }
        return false;
    }
    ...
    private String updateFocusForVideo() {
        ...
        if( this.supported_focus_values != null && mCameraController != null && is_video ) {
    ...
    private void setFlash(String flash_value) {
        ...
        cancelAutoFocus();
        mCameraController.setFlashValue(flash_value);
    }
    ...
    private void setFocusValue(String focus_value, boolean auto_focus) {
        ...
        mCameraController.setFocusValue(focus_value);
    ...
    private void setupContinuousFocusMove() {
        ...
        if( MyDebug.LOG )
            Log.d(TAG, "focus_value is " + focus_value);
        if( mCameraController != null && focus_value != null && focus_value.equals("focus_mode_continuous_picture") && !this.is_video ) {
            if( MyDebug.LOG )
                Log.d(TAG, "set continuous picture focus move callback");
                mCameraController.setContinuousFocusMoveCallback(new CameraController.ContinuousFocusMoveCallback() {
                @Override
        ...
        else if( mCameraController != null ) {
            if( MyDebug.LOG )
                Log.d(TAG, "remove continuous picture focus move callback");
                mCameraController.setContinuousFocusMoveCallback(null);
        }
    }
    ...
    public void toggleWhiteBalanceLock() {
        ...
        if( is_white_balance_lock_supported ) {
            ...
            cancelAutoFocus();
            mCameraController.setAutoWhiteBalanceLock(is_white_balance_locked);
        }
    }
    ...
    public void toggleExposureLock() {
        ...
        if( is_exposure_lock_supported ) {
            ...
            cancelAutoFocus();
            mCameraController.setAutoExposureLock(is_exposure_locked);
        }
    }
    ...
    public void takePicturePressed(boolean photo_snapshot, boolean continuous_fast_burst) {
        ...
        if( is_video && isVideoRecording() && !photo_snapshot ) {
            ...
            else if( !is_video && mCameraController.getBurstType() == CameraController.BurstType.BURSTTYPE_FOCUS && mCameraController.isCapturingBurst() ) {
                mCameraController.stopFocusBracketingBurst();
            ...
            public void run() {
                ...
                activity.runOnUiThread(new Runnable() {
                    public void run() {
                        ...
                        if( mCameraController != null && takePictureTimerTask != null )
                            takePicture(false, false, false);
                        else {
                            if( MyDebug.LOG )
    ...
    private void flashVideo() {
        ...
        // getFlashValue() may return "" if flash not supported!
        String flash_value = mCameraController.getFlashValue();
        if( flash_value.length() == 0 )
            return;
        ...
        if( flash_value.equals("flash_torch") ) {
            ...
            cancelAutoFocus();
            mCameraController.setFlashValue(flash_value_ui);
            return;
        }
        // turn on torch
        cancelAutoFocus();
        mCameraController.setFlashValue("flash_torch");
        try {
            Thread.sleep(100);
        }
    ...
    private void startVideoRecording(final boolean max_filesize_restart) {
        ...
        if( info == null ) {
            ...
            if( MyDebug.LOG )
                ...
                mCameraController.enableShutterSound(enable_sound); // Camera2 API can disable video sound too
            ...
            mCameraController.initVideoRecorderPrePrepare(local_video_recorder);
    ...
    private void videoRecordingStarted(boolean max_filesize_restart) {
        ...
        if( this.using_face_detection && !this.using_android_l ) {
            ...
            mCameraController.startFaceDetection();
            faces_detected = null;
        }
                ...
                public void run() {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        public void run() {
                            // we run on main thread to avoid problem of camera closing at the same time
                            // but still need to check that the camera hasn't closed or the task halted, since TimerTask.run() started
                            if( mCameraController != null && flashVideoTimerTask != null )
                                flashVideo();
                            else {
                                if( MyDebug.LOG )
                    ...
                    if( battery_frac <= 0.03 ) {
                        ...
                        activity.runOnUiThread(new Runnable() {
                            public void run() {
                                // we run on main thread to avoid problem of camera closing at the same time
                                // but still need to check that the camera hasn't closed or the task halted, since TimerTask.run() started
                                if( mCameraController != null && batteryCheckVideoTimerTask != null ) {
                                    stopVideo(false);
    ...
    private void takePhoto(boolean skip_autofocus, final boolean continuous_fast_burst) {
        ...
        if( autofocus_in_continuous_mode ) {
            ...
            if( wait_for_focus ) {
                mCameraController.setCaptureFollowAutofocusHint(true);
            }
            else {
                takePhotoWhenFocused(continuous_fast_burst);
            }
        }
        else if( mCameraController.focusIsContinuous() ) {
            if( MyDebug.LOG )
            ...
            // note: if the user touches to focus in continuous mode, we camera controller may be in auto focus mode, so we should only enter this codepath if the mCameraController is in continuous focus mode
            ...
            mCameraController.autoFocus(autoFocusCallback, true);
        }
            ...
            boolean wait_for_focus;
            // n.b., we check focus_value rather than mCameraController.supportsAutoFocus(), as we want to discount focus_mode_locked
            synchronized(this) {
                if( focus_success == FOCUS_WAITING ) {
    ...
    private void takePhotoWhenFocused(boolean continuous_fast_burst) {
        ...
        CameraController.PictureCallback pictureCallback = new CameraController.PictureCallback() {
            ...
            public void onStarted() {
                ...
                if( applicationInterface.getBurstForNoiseReduction() && applicationInterface.getNRModePref() == ApplicationInterface.NRModePref.NRMODE_LOW_LIGHT ) {
                    if( mCameraController.getBurstTotal() >= CameraController.N_IMAGES_NR_DARK_LOW_LIGHT ) {
            ...
            public void onCompleted() {
                ...
                if( mCameraController != null && focus_value != null && ( focus_value.equals("focus_mode_continuous_picture") || focus_value.equals("focus_mode_continuous_video") ) ) {
                    if( MyDebug.LOG )
                        Log.d(TAG, "cancelAutoFocus to restart continuous focusing");
                        mCameraController.cancelAutoFocus(); // needed to restart continuous focusing
                }

                if( mCameraController != null && mCameraController.getBurstType() == CameraController.BurstType.BURSTTYPE_CONTINUOUS ) {
                    if( MyDebug.LOG )
        ...
        {
            mCameraController.setRotation(getImageVideoRotation());
            ...
            if( MyDebug.LOG )
                Log.d(TAG, "enable_sound? " + enable_sound);
                mCameraController.enableShutterSound(enable_sound);
            if( using_android_l ) {
                ...
                if( MyDebug.LOG )
                    ...
                    mCameraController.setUseExpoFastBurst( use_camera2_fast_burst );
            }
            if( continuous_fast_burst ) {
                mCameraController.setBurstType(CameraController.BurstType.BURSTTYPE_CONTINUOUS);
            }

            if( MyDebug.LOG )
                Log.d(TAG, "about to call takePicture");
                mCameraController.takePicture(pictureCallback, errorCallback);
    ...
    private void tryAutoFocus(final boolean startup, final boolean manual) {
        ...
        if( mCameraController == null ) {
            ...
            if( manual && !is_video && mCameraController.focusIsContinuous() && supportedFocusValue("focus_mode_auto") ) {
                if( MyDebug.LOG )
                    Log.d(TAG, "switch from continuous to autofocus mode for touch focus");
                    mCameraController.setFocusValue("focus_mode_auto"); // switch to autofocus
            ...
            if( mCameraController.supportsAutoFocus() ) {
                if( MyDebug.LOG )
                ...
                mCameraController.autoFocus(autoFocusCallback, false);
                count_cameraAutoFocus++;
    ...
    /** If the user touches the screen in continuous focus mode, we switch the mCameraController to autofocus mode.
     *  After the autofocus completes, we set a reset_continuous_focus_runnable to switch back to the mCameraController
    ...
    private void continuousFocusReset() {
        ...
        if( mCameraController != null && autofocus_in_continuous_mode ) {
            autofocus_in_continuous_mode = false;
            // check again
            String current_ui_focus_value = getCurrentFocusValue();
            if( current_ui_focus_value != null && !mCameraController.getFocusValue().equals(current_ui_focus_value) && mCameraController.getFocusValue().equals("focus_mode_auto") ) {
                mCameraController.cancelAutoFocus();
                if( MyDebug.LOG )
                    Log.d(TAG, "switch back to: " + current_ui_focus_value);
                    mCameraController.setFocusValue(current_ui_focus_value);
            }
            else {
                if( MyDebug.LOG )
    ...
    private void ensureFlashCorrect() {
        ...
        if( set_flash_value_after_autofocus.length() > 0 && mCameraController != null ) {
            if( MyDebug.LOG )
                Log.d(TAG, "set flash back to: " + set_flash_value_after_autofocus);
                mCameraController.setFlashValue(set_flash_value_after_autofocus);
            set_flash_value_after_autofocus = "";
        }
    }
    ...
    private void autoFocusCompleted(boolean manual, boolean success, boolean cancelled) {
        ...
        if( manual && mCameraController != null && autofocus_in_continuous_mode ) {
            String current_ui_focus_value = getCurrentFocusValue();
            if( MyDebug.LOG )
    ...
    public void startCameraPreview() {
        ...
        if( mCameraController != null && !this.isTakingPhotoOrOnTimer() && !is_preview_started ) {
            if( MyDebug.LOG )
                Log.d(TAG, "starting the camera preview");
            {
                if( MyDebug.LOG )
                    Log.d(TAG, "setRecordingHint: " + is_video);
                    mCameraController.setRecordingHint(this.is_video);
            }
            setPreviewFps();
            try {
    ...
    public boolean getOpticalStabilization() {
        ...
        return mCameraController.getOpticalStabilization();
    }
    ...
    public boolean getVideoStabilization() {
        ...
        return mCameraController.getVideoStabilization();
    }
    ...
    public List<String> getSupportedCapabilities() {
        return this.supported_capabilitys;
    }
    ...
    public String getISOKey() {
        if( MyDebug.LOG )
            Log.d(TAG, "getISOKey");
        return mCameraController == null ? "" : mCameraController.getISOKey();
    }
    ...
    public int getCurrentExposure() {
        ...
        return mCameraController.getExposureCompensation();
    }
    ...
    public List<CameraController.Size> getSupportedPictureSizes(boolean check_supported) {
        ...
        boolean is_burst = ( mCameraController != null && mCameraController.isBurstOrExpo() );
    ...
    public String getCameraAPI() {
        if( mCameraController == null )
            return "None";
        return mCameraController.getAPI();
    }
    ...
    private void updateParametersFromLocation() {
        ...
        if( mCameraController != null ) {
            ...
            if( store_location && applicationInterface.getLocation() != null ) {
                ...
                mCameraController.setLocationInfo(location);
            }
            else {
                if( MyDebug.LOG )
                    Log.d(TAG, "removing location data from parameters...");
                    mCameraController.removeLocationInfo();
            }
        }
    }
